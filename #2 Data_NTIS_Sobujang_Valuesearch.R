...
# 극단치 기업 제거 - 영업 이익률 
# 장비 엔젤로보틱스 통제 / 4358600568
# 장비 (주) 코윈디에스티 통제 /1318193141
# 장비 토모큐브 통제 / 5148800190
# 장비 에이치로보틱스 통제 / 4078131509

# 부품 주식회사뉴라텍 통제 / 3148656616
# 부품 (주)테크트랜스 처치 / 5158141685
# 부품 유우일렉트로닉스 처치/ 2068608415
# 부품 (주) 니어스랩 처치 / 1298700068
# 부품 (주) 에스오에스랩 처치 / 4188800382

# 소재 칸토텐카화인프로덕츠 틍제 / 8828100643
...
merged_data <- merged_data %>%
  filter(!(사업자번호_clean %in% opm_outlier_biz))
...
cat("\n")
cat("==============================================================================\n")
cat("  OPM 극단치 기업 제거 완료\n")
cat("==============================================================================\n")
cat(sprintf("  제거 전 : %d개\n", n_before))
cat(sprintf("  제거 후 : %d개\n", n_after))
cat(sprintf("  제거 수 : %d개  (목표 13개)\n", n_removed))
if (n_removed == 13) {
  cat("  ✓ 13개 기업 모두 정상 제거됨\n")
} else {
  cat("  ⚠ 제거 수가 13개와 다릅니다. 사업자번호_clean 컬럼 형식을 확인하세요.\n")
  still_in <- merged_data %>%
    filter(사업자번호_clean %in% opm_outlier_biz) %>%
    select(기업명, 사업자번호_clean)
  if (nrow(still_in) > 0) { cat("  남아있는 기업:\n"); print(still_in) }
}
cat("==============================================================================\n\n")
